...
export class TableDisplay {
  ...
  public render(data: Record<string, any>[], columns: Column[]): string {
    ...
    if (data.length === 0) {
      ...
      rowsOutput = data.map((row, index) => this.renderRow(row, adjustedColumns, index)).join('\n');
    }
  ...
  private formatHeader(text: string, column: Column): string {
    ...
    if (this.truncate && formattedText.length > availableWidth) {
      formattedText =
        formattedText.substring(0, availableWidth - this.truncateChar.length) + this.truncateChar;
    }
  ...
  private formatCell(value: any, column: Column, rowIndex = 0): string {
    ...
    if (this.truncate && formattedValue.length > availableWidth) {
      formattedValue =
        formattedValue.substring(0, availableWidth - this.truncateChar.length) + this.truncateChar;
    }
   ...
   */
  private adjustColumnsToTerminalWidth(
    columns: Column[],
    ...
    if (currentWidth > availableWidth) {
      ...
      const nonResizableWidth = nonResizableColumns.reduce((sum, column) => sum + column.width, 0);
      ...
      const totalResizableWidth = resizableColumns.reduce((sum, column) => sum + column.width, 0);
