...
describe('PR Review Command', () => {
  ...
  beforeEach(() => {
    ...
    const mockRepositoryService = (RepositoryService as jest.Mock).mock.results[0]?.value || 
      (new (RepositoryService as jest.Mock)());
    
    const mockContentService = (ContentService as jest.Mock).mock.results[0]?.value || 
      (new (ContentService as jest.Mock)());
  ...
  it('should display PR diff and return without error', async () => {
    ...
    expect(console.log).toHaveBeenCalledWith(expect.stringContaining('Reviewing changes in PR #123:'));
  });
  ...
  it('should handle invalid PR numbers', async () => {
    await expect(reviewPullRequest('invalid' as any)).rejects.toThrow('Invalid pull request number');
  ...
  it('should handle API errors when getting PR details', async () => {
    ...
    mockGetPullRequest.mockRejectedValue(
      new Error('API error')
    );
  ...
  it('should handle API errors when getting PR diff', async () => {
    ...
    mockGetPullRequestDiff.mockRejectedValue(
      new Error('Diff API error')
    );
