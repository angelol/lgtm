...
describe('PR Approval Command', () => {
  ...
  it('should approve a PR with successful CI status', async () => {
    ...
    expect(console.log).toHaveBeenCalledWith(expect.stringContaining('Successfully approved PR #123'));
  });
  ...
  it('should handle failing CI status with confirmation', async () => {
    ...
    const confirmFailingCiMock = require('../../../src/ui/confirm.js').confirmFailingCi as jest.Mock;
  ...
  it('should handle invalid PR numbers', async () => {
    await expect(approvePullRequest('invalid' as any)).rejects.toThrow('Invalid pull request number');
